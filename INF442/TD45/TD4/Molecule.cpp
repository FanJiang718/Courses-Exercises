...
void boundingBox(double BB[3][2], std::vector<Boule> mol) {
	...
	for(int i = 0; i< 3; i++){
		double tmp = it->centre[i] + it->radius;
		if(tmp > BB[i][1]) BB[i][1] = tmp;
		double tmp2  = it->centre[i] - it->radius;
		if(tmp2 < BB[i][0]) BB[i][0] = tmp2;
	}
	}
}
...
void randomPoint(double pt[3], const double BB[3][2]) {
	...
	pt[0] = rand()/(double)RAND_MAX*(BB[0][1]-BB[0][0]) +BB[0][0];
	pt[1] = rand()/(double)RAND_MAX*(BB[1][1]-BB[1][0]) + BB[1][0];
	pt[2] = rand()/(double)RAND_MAX*(BB[2][1]-BB[2][0]) + BB[2][0];
