...
def estimMultiniveaux(eps,P):
    ...
    for l in range(1,L+1):
        ...
        for k in range(2**(l-1)):
            ...
            Se_gr = Se_gr*(1 + r*h_fin*2 + sig_fin*(g1+g2))
            
        ###################################################################
        # Ajouter a la variable estim_multi la contribution du terme
        # au niveau l
        # (la moyenne par lignes des differences entre les deux schemas)
        ###################################################################
        estim_multi = estim_multi + np.mean( (K-Se_fin)*(K>Se_fin)-(K-Se_gr)*(K>Se_gr) , axis = 0)
