...
lambda_opt = np.mean((X-np.mean(X))*(Y-np.mean(Y)))/np.var(X)

lam = (3-np.e)*6
print("lambda theorique" + str(lam))
...
U = np.random.rand(N)
Y = np.exp(U)
Ycontrole =lambda_opt* m_r + Y - lambda_opt*(1+U)
############################################
...
#############################################
## Pour les histogrammes: calculer les
## erreurs des deux estimateurs I_N et I_N controle
#############################################
erreur_N = np.mean(Y,axis = 1)-Esp_gY

erreur_controle_N = np.mean(Ycontrole,axis = 1)- Esp_gY
