...
public class AffichageHexadecimal {
  ...
  public static boolean allumeSegment(int segment, int chiffre) {
	  if(segment == 0){
		  if(chiffre == 0 || chiffre==2 ||chiffre ==3|| (chiffre >=5 && chiffre <=10)||chiffre == 12||chiffre ==14 ||chiffre ==15 )
			  return true;
		  else
			  return false;
	  }
	  else if(segment ==1){
		  if((chiffre>=0 && chiffre<=4)||(chiffre>=7 &&chiffre <=10)||chiffre ==13)
			  return true;
		  else 
			  return false;		  
	  }
		  ...
		  if(chiffre == 0|| chiffre == 2|| chiffre == 6 || chiffre == 8|| (chiffre >=10 &&chiffre <= 15))
			  return true;
		  else
			  return false;
	  }
    ...
    // MODIFIER pour la valeur de retour correcte
  }
