...
if featureSelection == True:
    ...
    gain_info = infogain(X,Y)
    gain_chi = chiSQ(X,Y)

    index_chi = np.argsort(gain_chi)[::-1]
    index_info = np.argsort(gain_info)[::-1]

    ########## ADD YOUR CODE HERE #######################################
    # Compute Kendall tau correlation
    ## TODO: Compute the Kendall tau correlation of the features' lists produced
    # by the two feature selection measures
    tau = sts.kendalltau(index_chi, index_info)
    print("Kendall tau correlation: {}".format(tau))

    #####################################################################

    # Select the top num_feat features
    X = X[:,index_info[:num_feat]]
