...
for t in range(T):
    ...
    indic = (y_pred!=y_train)
    gamma = np.sum(w*indic)/np.sum(w)
    alpha = np.log((1-gamma)/gamma)
    w = w*np.exp(alpha*indic)
    training_scores += alpha*y_pred
    y_pred_test = tree.predict(X_test)
    test_scores += alpha*y_pred_test
    
    training_errors.append(np.mean(np.sign(training_scores)!=y_train))
    test_errors.append(np.mean(np.sign(test_scores)!=y_test))
...
test_errors = []
for d in D:
    err_training, err_test = boost_tree(d)
    training_errors.append(err_training[-1])
    test_errors.append(err_test[-1])
...
plt.legend()
plt.title("test error with different D")
plt.show()